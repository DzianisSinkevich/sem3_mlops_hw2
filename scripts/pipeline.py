...
# from mlops import PipelineDecorator
import subprocess
import os
...
@PipelineDecorator.component(cache=False, execution_queue="default")
def process_data():
    subprocess.run(["python", "./scripts/df_process.py"], check=True)
...
def train_model_1():
    try:
        ...
        if current_task:
            ...
            subprocess.run(["python", "./scripts/train_model.py", "--model-type", "model_1"], check=True)
...
def train_model_2():
    try:
        ...
        if current_task:
            ...
            # Запускаем скрипт с помощью subprocess
            subprocess.run(["python", "./scripts/train_model.py", "--model-type", "model_2"], check=True)
